...
must_haves:
  truths:
    - "Claude can parse raw OCR text and return structured data matching the document type schema"
  ...
  artifacts:
    - path: "src/lib/ocr/structured-extractor.ts"
      provides: "Claude-powered structured data extraction from OCR text"
      contains: "extractStructuredData"
    - path: "src/lib/ocr/extraction-prompts.ts"
      provides: "Document-type-specific extraction prompts for Claude"
...
<objective>
Build the Claude-powered structured data extractor that takes raw OCR text and produces typed, confidence-scored field extractions for each document type.

Purpose: Raw OCR text alone is useless for KYC — we need structured fields (name, DOB, address, document numbers) with confidence scores. Claude is ideal for this because it can reason about ambiguous text, handle OCR errors, and assign meaningful confidence levels.
...
<task type="auto">
  ...
  <action>
    Create `src/lib/ocr/extraction-prompts.ts` with extraction prompts for each document type. These prompts instruct Claude to extract specific fields with confidence scores.
    ...
    IMPORTANT: The prompts ask Claude to return ONLY JSON, no markdown wrapping. This makes parsing reliable. The confidence instructions are shared across all prompts to ensure consistent scoring.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Build Claude structured data extractor with confidence scoring</name>
  <files>src/lib/ocr/structured-extractor.ts</files>
  <action>
    Create `src/lib/ocr/structured-extractor.ts` — uses Claude to extract structured data from raw OCR text:
    ...
    /**
     * Extract structured data from raw OCR text using Claude.
    ...
    export async function extractStructuredData(
      ...
      const message = await client.messages.create({
        ...
        messages: [
          {
            role: 'user',
            content: `Here is the OCR text extracted from a ${documentType.replace('_', ' ')} document:\n\n---\n${ocrText}\n---\n\n${prompt}`,
          },
        ],
      });
      ...
      // Parse JSON response - strip any markdown code fences if Claude adds them
    ...
    IMPORTANT NOTES:
    - Uses Claude Sonnet 4.6 (model ID: `claude-sonnet-4-6-20250514`) — matches the project stack decision for reasoning tasks.
    - The JSON parsing includes cleanup for markdown code fences that Claude sometimes adds despite instructions.
  ...
  <verify>
    ...
    - Imports from @anthropic-ai/sdk (not a custom wrapper)
    - Uses claude-sonnet-4-6 model (not hardcoded to an old model)
    - No hardcoded API keys
  </verify>
  <done>
    Claude-powered structured data extractor built. Takes raw OCR text + document type, calls Claude with type-specific prompts, parses the JSON response into typed ExtractedDocumentData with per-field confidence scores. Handles all four document types with graceful degradation for missing fields.
  </done>
</task>
...
3. Structured extractor calls Claude and returns typed data
...
<success_criteria>
- Claude extracts structured data from OCR text for all four document types (DOC-02)
