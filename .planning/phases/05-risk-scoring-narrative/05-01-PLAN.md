...
<task type="auto">
  ...
  <action>
    Create `src/lib/agents/scoring-engine.ts` — a pure TypeScript module with NO GenAI SDK or LLM dependencies. This is deterministic scoring logic that can be unit tested without API calls.
  ...
  <action>
    ...
    **Agent handler function:**
    ...
    export async function riskScorerHandler(
      ...
      // Step 2: Use Gemini to generate a human-readable scoring summary
      // that explains the risk factors in compliance language
      const model = client.getGenerativeModel({ model: config.model });
      const result = await model.generateContent(`You are a KYC/AML risk scoring analyst. Analyze the following risk assessment and write a 2-3 sentence scoring summary suitable for a compliance officer.
...
Write a concise summary explaining the key risk drivers and whether this case warrants closer scrutiny. Be specific about which factors contributed most.`);
      ...
      const scoringSummary = result.response.text() || 'Unable to generate scoring summary.';
    ...
    **Registration function:**
    ...
    KEY DESIGN: The scoring engine does the math (deterministic, testable), and Gemini provides the human-readable interpretation. This separation means:
    ...
    The handler signature matches what `runAgent` expects: `(input: TInput, client: GoogleGenerativeAI, config: AgentConfig) => Promise<TOutput>`. The orchestrator calls `runAgent('risk_scorer', input, AGENT_CONFIGS.risk_scorer, getHandler('risk_scorer'))`.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
    - `riskScorerHandler` signature matches `(input: RiskScorerInput, client: GoogleGenerativeAI, config: AgentConfig) => Promise<RiskScorerOutput>`
    ...
    - Handler uses Gemini API only for the scoring_summary text
  ...
  <done>Risk Scorer agent handler built. Deterministic scoring via scoring-engine + LLM-generated summary via Gemini. Registers with orchestrator via registerAgent. Follows Phase 2 base-agent pattern exactly.</done>
</task>
