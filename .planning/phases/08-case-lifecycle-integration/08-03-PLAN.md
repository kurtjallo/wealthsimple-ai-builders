...
<task type="auto">
  ...
  <action>
    ...
    **Delay integration — wrap the `processCase` callback to inject delays between stages:**
    ...
    IMPORTANT: The delay simulator adds time BETWEEN stages, not to the agent execution itself. If real agents already take 2+ seconds (Mistral OCR, Gemini API calls), you may want to reduce or skip delays. Add a check: if `result.duration_ms > 2000`, skip the simulated delay for that agent.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
