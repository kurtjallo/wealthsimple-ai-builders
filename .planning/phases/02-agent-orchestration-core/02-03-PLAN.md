...
<objective>
Create realistic stub implementations for all 5 specialized agents. These stubs return properly typed, realistic-looking data with simulated delays, enabling the full pipeline to run end-to-end without requiring Gemini API calls.
...
<task type="auto">
  ...
  <action>
    ...
    - Is a function matching the handler signature: `(input: TInput, client: GoogleGenerativeAI, config: AgentConfig) => Promise<TOutput>`
    ...
    - Does NOT call the Gemini API (that's for later phases)
