...
must_haves:
  ...
  artifacts:
    - path: "src/lib/agents/base-agent.ts"
      provides: "Base agent runner function that wraps Gemini API calls with timing, error handling, and typed results"
      contains: "runAgent"
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: The `GoogleGenerativeAI` client is instantiated once at module level — it reads `GEMINI_API_KEY` from env. Do NOT pass the key explicitly.
